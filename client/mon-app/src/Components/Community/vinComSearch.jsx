...
export default class VinCommunitySearch extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      name: '',
    ...
    newPage(page){
      ...
      axios.post(`http://localhost:3000/vinCommunity/search`, { vin })
        .then(res => {
    ...
    render() {
      ...
      const vins = this.state.vins.map((vin) => {
        return <ListGroup.Item key={vin._id}><Link to={{
          ...
          owned:true,
        }}  >{vin.name} - Appartient à {vin.owner} - Notes : ({vin.rating} / 5)</Link></ListGroup.Item>
      })
        ...
        <Container>
          <form onSubmit={this.handleSubmit}>
            <InputGroup className="mb-3">
              <FormControl
                placeholder="Chercher un vin de la communauté"
                aria-label="Chercher un vin de la communauté"
